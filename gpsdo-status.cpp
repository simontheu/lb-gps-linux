/*******************************************************
 ...
 which use HIDAPI.
********************************************************/
// what Mr. Ott said (see above). 
// (was) hidtest.cpp (now w/changes) gpsdo-status          CSylvain  2019-04-15
	...
	while (cur_dev) {
		if (cur_dev->vendor_id == VID_LB_USB) {
			...
			printf("  Firmware:      %hx.%hx\n",
				   ((cur_dev->release_number & 0xf00)>>8), (cur_dev->release_number & 0xff));
...
/*
	...
	else printf("Serial Number String: (%d) %ls\n", wstr[0], wstr);

	// Read Indexed String 0
	wstr[0] = 0x0000;
	res = hid_get_indexed_string(handle, 0, wstr, MAX_STR);
	if (res < 0) {
		printf("Unable to read indexed string 0\n");
	}
	else printf("Indexed String 0: %ls\n", wstr);
